    ...
    namespace PurePursuitOA
    {
      ...
      struct Task: public DUNE::Control::PathController
      {
        ...
        step(const IMC::EstimatedState& state, const TrackingState& ts)
        {
          ...
          if (m_ts.nearby && (clic % 20 == 0))
          {
            // war("CCCCCCCCCCCC");
            // Nota: Se já restaurou na lógica acima, isto não vai executar novamente
            if (currAvoidState)
            {
              // durante avoidance, o destino intermédio pode ser atingido sem mudar de estado;
              // enviar nova referência evita o timeout "expected new path control reference"
              m_ts.nearby = false;
              dsptch = true;
            }
            else if (epIsSet)
